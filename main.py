...
def quick_mail(n, export='csv'):
    samples = [_email.gen_email() for i in range (n)]

    if export == 'csv':
        csv_file_path = os.path.join(export_path, f"tpme_{datetime.datetime.now()}.csv")
        with open(csv_file_path, "w", newline="") as csv_file:
            csv_writer = csv.writer(csv_file)
            csv_writer.writerow(["Emails generated by TPME - https://github.com/wlinds/tpme"])
            for sample in samples:
                csv_writer.writerow([sample])

    elif export == 'json':
        json_file_path = os.path.join(export_path, f"tpme_{datetime.datetime.now()}.json")
        with open(json_file_path, "w") as json_file:
            json.dump({"emails": samples}, json_file, indent=2)
...
if __name__ == '__main__':
    ...
    print(quick_mail(1000,export='csv'))